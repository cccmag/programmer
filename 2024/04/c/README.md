...
雖然 c4 的作者並沒有給其虛擬機一個名稱，但是為了方便，我們將 c4 的虛擬機稱為 vm4。
...
vm4 虛擬機是一種堆疊機，包含累積器 a, 堆疊暫存器 sp, 框架暫存器 bp 。其中所有 +-*/ 等運算幾乎都是對 a 進行的，而 bp 則可被用來存取區域變數。
...
LLA : load local address 載入區域變數 (原本叫 LEA)              a = (int)(bp + *pc++); 
...
為了讓大家更了解 c4 ，我們將其虛擬機程式碼抽出來，然後直接用類似組合語言的方式，產生一段可以印出 hello 的程式碼去執行，其程式碼如下：
...
完整的程式碼位於 [01-asmHello/c4.c](01-c4vm/01-asmHello/c4.c) 中，編譯執行結果如下

```bash
ccckmit@asus MINGW64 /d/ccc/程式人雜誌/202402/01-c4vm/01-asmHello
$ ./c4
1> ENT  0
2> IMM  4215121
3> PSH
...
以上程式直接用 `*e++ = ENT; *e++ = 0;` ，其實我們可以用定義指令產生的巨集 E1, E2 等函數如下
...
如果我們將 fib.c 產生的虛擬機程式碼，直接用類似方式寫入，那麼就可以進行費氏數列的計算
...
然後調用下列虛擬機程式去執行，該程式有事先準備好堆疊，並且可以傳入 argc, argv 等參數 (只是我們沒有用它)
...
```
ccckmit@asus MINGW64 /d/ccc/程式人雜誌/202402/01-c4vm/02b-asmFib2
$ gcc c4.c -o c4
...
透過以上的範例，希望大家對 vm4 (c4 的虛擬機) 能有更清楚的理解！